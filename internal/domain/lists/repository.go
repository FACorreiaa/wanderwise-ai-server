...
type Repository interface {
	...
	GetList(ctx context.Context, listID uuid.UUID) (List, error)
	UpdateList(ctx context.Context, list List) error
	GetSubLists(ctx context.Context, parentListID uuid.UUID) ([]*List, error)
	GetListItems(ctx context.Context, listID uuid.UUID) ([]*ListItem, error)

	// Generic list item methods (support all content types)
	GetListItemByID(ctx context.Context, listID, itemID uuid.UUID) (ListItem, error)
	...
	GetUserSavedLists(ctx context.Context, userID uuid.UUID) ([]*List, error)

	// Content type specific methods
	GetListItemsByContentType(ctx context.Context, listID uuid.UUID, contentType ContentType) ([]*ListItem, error)

	// Search and filtering
	SearchLists(ctx context.Context, searchTerm, category, contentType, theme string, cityID *uuid.UUID) ([]*List, error)

	// Legacy POI-specific methods (for backward compatibility)
	GetListItem(ctx context.Context, listID, itemID uuid.UUID, contentType string) (ListItem, error)
	AddListItem(ctx context.Context, item ListItem) error
	...
	GetUserLists(ctx context.Context, userID uuid.UUID, isItinerary bool) ([]*List, error)
}
...
func (r *RepositoryImpl) CreateList(ctx context.Context, list List) error {
	query := `
        INSERT INTO lists (
...
func (r *RepositoryImpl) GetList(ctx context.Context, listID uuid.UUID) (List, error) {
	query := `
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return List{}, fmt.Errorf("list not found: %w", err)
		}
		r.logger.Error("Failed to get list", zap.Error(err))
		return List{}, fmt.Errorf("failed to get list: %w", err)
	}
	return list, nil
}

// GetSubLists retrieves all sub-lists with a given parent_list_id
func (r *RepositoryImpl) GetSubLists(ctx context.Context, parentListID uuid.UUID) ([]*List, error) {
	query := `
        SELECT id, user_id, name, description, image_url, is_public, is_itinerary,
               parent_list_id, city_id, view_count, save_count, created_at, updated_at
        FROM lists
	...
	for rows.Next() {
		...
		if err != nil {
			r.logger.Error("Failed to scan sub-list", zap.Error(err))
			return nil, fmt.Errorf("failed to scan sub-list: %w", err)
		}
		subLists = append(subLists, &list)
	}
	if err = rows.Err(); err != nil {
		r.logger.Error("Error iterating sub-list rows", zap.Error(err))
...
func (r *RepositoryImpl) GetListItems(ctx context.Context, listID uuid.UUID) ([]*ListItem, error) {
	query := `
	...
	if err != nil {
		r.logger.Error("Failed to get list items", zap.Error(err))
	...
	for rows.Next() {
		...
		if err != nil {
			r.logger.Error("Failed to scan list item", zap.Error(err))
	...
	if err = rows.Err(); err != nil {
		r.logger.Error("Error iterating list item rows", zap.Error(err))
...
func (r *RepositoryImpl) AddListItem(ctx context.Context, item ListItem) error {
	var poiID *uuid.UUID
	// Only set poi_id for POI content type to avoid foreign key constraint violations
	if item.ContentType == ContentTypePOI {
		poiID = &item.ItemID
	}
	...
	if err != nil {
		r.logger.Error("Failed to add list item", zap.Error(err))
...
func (r *RepositoryImpl) DeleteListItem(ctx context.Context, listID, itemID uuid.UUID, contentType string) error {
	...
	if err != nil {
		r.logger.Error("Failed to delete list item", zap.Error(err))
...
func (r *RepositoryImpl) DeleteList(ctx context.Context, listID uuid.UUID) error {
	...
	if err != nil {
		r.logger.Error("Failed to delete list", zap.Error(err))
...
func (r *RepositoryImpl) UpdateList(ctx context.Context, list List) error {
	query := `
        UPDATE lists
	...
	if err != nil {
		r.logger.Error("Failed to update list", zap.Error(err))
...
func (r *RepositoryImpl) GetListItem(ctx context.Context, listID, itemID uuid.UUID, contentType string) (ListItem, error) {
	query := `
        SELECT list_id, item_id, content_type, position, notes, day_number, time_slot, duration, 
               source_llm_interaction_id, item_ai_description, created_at, updated_at
        FROM list_items
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return ListItem{}, fmt.Errorf("list item not found: %w", err)
		}
		r.logger.Error("Failed to get list item", zap.Error(err))
		return ListItem{}, fmt.Errorf("failed to get list item: %w", err)
	}
	if dayNumber.Valid {
		dn := int(dayNumber.Int32)
		item.DayNumber = &dn
...
func (r *RepositoryImpl) UpdateListItem(ctx context.Context, item ListItem) error {
	query := `
        UPDATE list_items
	...
	if err != nil {
		r.logger.Error("Failed to update list item", zap.Error(err))
...
func (r *RepositoryImpl) GetUserLists(ctx context.Context, userID uuid.UUID, isItinerary bool) ([]*List, error) {
	query := `
        SELECT id, user_id, name, description, image_url, is_public, is_itinerary,
               parent_list_id, city_id, view_count, save_count, created_at, updated_at
        FROM lists
	...
	for rows.Next() {
		...
		if err != nil {
			r.logger.Error("Failed to scan list", zap.Error(err))
			return nil, fmt.Errorf("failed to scan list: %w", err)
		}
		lists = append(lists, &list)
	}
	if err = rows.Err(); err != nil {
		r.logger.Error("Error iterating list rows", zap.Error(err))
...
func (r *RepositoryImpl) GetListItemByID(ctx context.Context, listID, itemID uuid.UUID) (ListItem, error) {
	query := `
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return ListItem{}, fmt.Errorf("no list item found for list_id %s and item_id %s", listID, itemID)
		}
		r.logger.Error("Failed to get list item by ID", zap.Error(err))
		return ListItem{}, fmt.Errorf("failed to get list item: %w", err)
	}
	return item, nil
}
...
func (r *RepositoryImpl) DeleteListItemByID(ctx context.Context, listID, itemID uuid.UUID) error {
	...
	if err != nil {
		r.logger.Error("Failed to delete list item by ID", zap.Error(err))
		return fmt.Errorf("failed to delete list item: %w", err)
	}
	if result.RowsAffected() == 0 {
...
func (r *RepositoryImpl) SaveList(ctx context.Context, userID, listID uuid.UUID) error {
	...
	if err != nil {
		r.logger.Error("Failed to save list", zap.Error(err))
...
func (r *RepositoryImpl) UnsaveList(ctx context.Context, userID, listID uuid.UUID) error {
	...
	if err != nil {
		r.logger.Error("Failed to unsave list", zap.Error(err))
...
func (r *RepositoryImpl) GetUserSavedLists(ctx context.Context, userID uuid.UUID) ([]*List, error) {
	query := `
	...
	for rows.Next() {
		...
		if err != nil {
			r.logger.Error("Failed to scan saved list", zap.Error(err))
			return nil, fmt.Errorf("failed to scan saved list: %w", err)
		}
		lists = append(lists, &list)
	}
	if err = rows.Err(); err != nil {
		r.logger.Error("Error iterating saved list rows", zap.Error(err))
...
func (r *RepositoryImpl) GetListItemsByContentType(ctx context.Context, listID uuid.UUID, contentType ContentType) ([]*ListItem, error) {
	query := `
		SELECT list_id, item_id, content_type, position, notes, day_number, 
		       time_slot, duration, source_llm_interaction_id, item_ai_description, 
		       created_at, updated_at
	...
	if err != nil {
		r.logger.Error("Failed to get list items by content type", zap.Error(err))
...
func (r *RepositoryImpl) SearchLists(ctx context.Context, searchTerm, category, contentType, theme string, cityID *uuid.UUID) ([]*List, error) {
	query := `
